...
export default function Study() {
	...
	useEffect(() => {
		if (ready && userId) {
			...
			useDB
				.getState()
				.ensureUserCardsForAllWords(userId)
				.then(() => {
	...
	if (!current)
		return (
			<p>
				All caught up! No cards due. Try adding more words or come back later.
			</p>
		)
	...
	return (
		<div>
			...
			<Flashcard card={current} flipped={flipped} onFlip={onShow} />
			<div className="controls">
				{!flipped ? (
					<button className="btn btn-muted" onClick={onShow}>
						Show answer
					</button>
				) : (
					<>
						<button className="btn btn-danger" onClick={() => onMark(false)}>
							Wrong
						</button>
						<button className="btn btn-primary" onClick={() => onMark(true)}>
							Correct
						</button>
					</>
				)}

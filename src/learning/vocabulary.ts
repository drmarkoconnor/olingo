...
const thematicSeeds: Record<VocabDomain, VocabularySeed[]> = {
	food: [
		...
		['ho gia mangiato', 'I have already eaten', 'chunk', 'A2', 90],
	...
	cafe: [
		...
		['ho gia pagato', 'I have already paid', 'chunk', 'A2', 92],
	...
	shopping: [
		...
		['va bene cosi', 'that is fine', 'chunk', 'A2', 88],
	],
	travel: [
	...
	culture: [
		...
		['mi e piaciuto', 'I liked it', 'chunk', 'A2', 92],
...
function domainsForScene(sceneId: string, programWeek = 1) {
...
export function getThematicVocabularyForScene(
	...
	const domains = domainsForScene(sceneId, options.programWeek)
	...
	for (const item of [...sceneItems, ...themed]) {
...
export async function loadVocabularyReviewQueue(
	...
	const pool = getThematicVocabularyForScene(sceneId, options)
	...
	const queue = pool
		...
		.filter((item) => item.dueKind !== 'future' || item.correctCount < 2)
